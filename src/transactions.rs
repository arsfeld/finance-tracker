...
use simplefin_bridge::{models::Transaction, SimpleFinBridge, AccountsParams};
...
pub async fn get_transactions_for_period(
    ...
    let transactions = accounts.accounts.iter().flat_map(|account| account.transactions.clone().unwrap_or_default()).collect();
